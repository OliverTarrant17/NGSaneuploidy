...
spec=matrix(c(
  ...
  'inbred', 'F',1,"character", "Inbreeding coefficient per sample e.g 0.2x3,0.1 is 0.2,0.2,0.2,0.1"
), byrow=TRUE, ncol=5)
opt <- getopt(spec)
...
# parse inbred
inbred <-c()
tmp <- strsplit(opt$inbred, split=",")[[1]]
for (j in 1:length(tmp)){
  tmp2 <- strsplit(tmp[j],split="x")[[1]]
  if (length(tmp2)>1) {
    inbred <- c(inbred, rep(as.double(tmp2[1]),times=tmp2[2]))
  } else {
    inbred <- c(inbred, as.double(tmp2[1]))
  }
}

rm(tmp); rm(tmp2)
if (max(inbred)>1 | min(inbred)<0) {
  cat("Inbred coefficient must be <=1 and >=0")
  q(status=1)
}

#check number of inbreeding coefficients is correct
if (length(inbred)!=length(ncopy)){
  cat("Number of inbreeding coefficients and samples don't align")
  q(status = 1)
}
# init files
...
rangeLams <- opt$depth*strtoi(ncopy[1])+(opt$depth*opt$errdepth*strtoi(ncopy[1])*c(-1,1)) # range of depths to accept
sampledLams <- runif(nsites, min=rangeLams[1], max=rangeLams[2])
sample_depths <- rpois(sampledLams,sampledLams)
depth <- matrix(sample_depths,nrow=1,ncol=nsites)
rm(sampledLams)
rm(rangeLams)
rm(sample_depths)
for (samp in 2:nsams){
  rangeLams <- opt$depth*strtoi(ncopy[samp])+(opt$depth*opt$errdepth*strtoi(ncopy[samp])*c(-1,1)) # range of depths to accept
  sampledLams <- runif(nsites, min=rangeLams[1], max=rangeLams[2])
  sample_depths <- rpois(sampledLams,sampledLams)
  depth<-rbind(depth,sample_depths)
  rm(sampledLams)
  rm(rangeLams)
  rm(sample_depths)
}

#rangeLams <- opt$depth+(opt$depth*opt$errdepth*c(-1,1)) # range of depths to accept
...
for (i in 1:opt$sites) {
  ...
  for (n in 1:nsams) { # cycle across samples
    ...
    if (ncopy[n]==2) {
      ...
      # priors: AA AC CC
      priors=c((1-inbred[n])*(pp^2)+inbred[n]*pp,(1-inbred[n])*2*pp*qq,(1-inbred[n])*(qq^2)+inbred[n]*qq)
    ...
    if (ncopy[n]==3) {
      ...
      priors=c((1-inbred[n])*(pp^3)+inbred[n]*pp,(1-inbred[n])*3*pp^2*qq,(1-inbred[n])*3*pp*qq^2,(1-inbred[n])*(qq^3)+inbred[n]*qq)
    ...
    if (ncopy[n]==4) {
      ...
      priors=c((1-inbred[n])*(pp^4)+inbred[n]*pp,(1-inbred[n])*4*pp^3*qq,(1-inbred[n])*6*pp^2*qq^2,(1-inbred[n])*4*pp*qq^3,(1-inbred[n])*(qq^4)+inbred[n]*qq)
    ...
    if (ncopy[n]==5) {
      ...
      priors=c((1-inbred[n])*(pp^5)+inbred[n]*pp,(1-inbred[n])*5*pp^4*qq,(1-inbred[n])*10*pp^3*qq^2,(1-inbred[n])*10*pp^2*qq^3,(1-inbred[n])*5*pp*qq^4,(1-inbred[n])*(qq^5)+inbred[n]*qq)
